...
3. **Champion clean architecture of tests**
    - Test design must be split into:
      Test principles (what to test)
      Implementation strategy (how to structure tests)
    - Architecture patterns (POM, Screenplay, etc.) are optional and context-driven, not enforced.
...
- No implicit assumptions about app structure
...
auth flow
environment
backend stubbing vs real API
application structure

👉 Do NOT guess. Ask concise clarifying questions.
## 7. Don’ts

- Do not hard-code waits – prefer locator.waitFor() or built-in auto-wait.
- Do NOT assume app structure without confirmation.
- Do NOT use infinite retries
- Do NOT silently ignore flaky tests
- Do NOT rely on vague “best practice” terminology
- Do not commit large trace bundles to VCS; upload as CI artifacts instead.
- Do not skip tests permanently; fix it.
...
repeatable
CI-stable
free of race-condition assumptions

Error Handling Standard > Failure classification:
...
max 1–2 only
never hide flakiness with retries
...
Must support:
MCP execution (if available)
standalone Playwright execution (CI/local fallback required)
No assumption of tooling availability
...
- [ ] All locators follows priority, are accessible, specific, and avoid strict mode violations
- [ ] No assumptions about app structure
- [ ] All tests are deterministic
- [ ] Auth/session strategy is explicitly defined
- [ ] Environment (CI/local/MCP) is confirmed
...
- [ ] Retry logic is limited and controlled (max 1–2)
- [ ] Flake rate is monitored and kept below 1%
- [ ] Tests validate real business outcomes, not implementation details
- [ ] Code coverage meets minimum thresholds for critical paths
- [ ] CI stability is prioritized over execution shortcuts### DTO (Data Transfer Object) Pattern
